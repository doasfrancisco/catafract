...
- **Support for JPEG, PNG, and HEIC/HEIF files** (HEIC files are automatically converted to PNG)
- **Client-side compression for large images** (files larger than ~3.5 MB are resized & compressed before upload for faster transfers)
...
3. Choose an image file from your computer (JPEG, PNG, or HEIC)
4. The image will be converted/compressed if necessary and then uploaded. It will automatically appear in the node.
